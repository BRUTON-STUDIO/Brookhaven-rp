...
local Z7xlib = {
	Themes = {
		...
		Z7X = {
			...
			["Color Hub 2"] = Color3.fromRGB(5, 12, 8),
			["Color Stroke"] = Color3.fromRGB(30, 110, 55),
			["Color Edge"] = Color3.fromRGB(57, 255, 106),
			["Color Theme"] = Color3.fromRGB(57, 255, 106),
			["Color Heading"] = Color3.fromRGB(57, 255, 106),
			["Color Text"] = Color3.fromRGB(255, 255, 255),
			["Color Dark Text"] = Color3.fromRGB(255, 255, 255),
			["Color Dark Purple"] = Color3.fromRGB(57, 255, 106),
			["Color Dark Greem"] = Color3.fromRGB(57, 255, 106),
			["Color Bubble"] = Color3.fromRGB(57, 255, 106)
		}
	},
	Info = {
		Version = "1.2.0"
...
local Settings = Z7xlib.Settings
local Flags = Z7xlib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(Z7xlib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then Z7xlib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then Z7xlib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then Z7xlib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, Z7xlib.Connection do
...
local GetFlag, SetFlag, CheckFlag do
	CheckFlag = function(Name)
		return Z7xlib.FlagsEnabled and type(Name) == "string" and Flags[Name] ~= nil
	end

	GetFlag = function(Name)
		return Z7xlib.FlagsEnabled and type(Name) == "string" and Flags[Name]
	end
...
local function VerifyTheme(Theme)
	for name,_ in pairs(Z7xlib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = Z7xlib.Themes[Z7xlib.Save.Theme]

local function AddEle(Name, Func)
	Z7xlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = Z7xlib.Elements[Ele](Instance, props, ...)
	return Element
end
...
function Z7xlib:Notify(Configs)
...
function Z7xlib:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(Z7xlib.Save.UISize)
	local MainFrame = InsertTheme(Create("ImageButton", ScreenGui, {
		Size = UDim2.fromOffset(UISizeX, UISizeY),
		Position = UDim2.new(0.5, -UISizeX/2, 0.5, -UISizeY/2),
		BackgroundColor3 = Color3.fromRGB(6, 16, 10),
...
local WindowBackground = Create("Frame", MainFrame, {
	...
	BackgroundColor3 = Color3.fromRGB(5, 12, 6),
	BackgroundTransparency = 1,
	ZIndex = 0
})
...
Make("Corner", WindowBackground)
	...
	local SidebarHeader = Create("Frame", Components, {
		Size = UDim2.new(0, Z7xlib.Save.TabSize, 0, 20),
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, Z7xlib.Save.TabSize, 1, -TopBar.Size.Y.Offset - 20),
...
local function CreateCyberpunkParticle()
	...
	local bodyTransparency = math.random(15, 40) / 100
	...
	CreateTween({Glow, "ImageTransparency", 0.55, 0.8})
	...
	for i = #ActiveParticles, 1, -1 do
		...
		if elapsed >= ParticleConfig.ParticleLifetime or
		   particle.Frame.Position.Y.Offset < -20 then
			...
			CreateTween({particle.Frame, "ImageTransparency", 1, 0.5})
			CreateTween({particle.Glow, "ImageTransparency", 1, 0.5})
...
local function SpawnCyberpunkSystem()
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			Z7xlib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("redz library V5.json", Z7xlib.Save)
		end
	end)

	ConnectSave(ControlSize2, function()
		Z7xlib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("redz library V5.json", Z7xlib.Save)
	end)
	...
	function Window:CloseBtn()
		local Dialog = Window:Dialog({
			Title = "Close",
			Text = "Are you sure the script is closed?",
			Options = {
				{"Confirm", function()
					ScreenGui:Destroy()
				end},
...
function Window:AddMinimizeButton(Configs)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			Z7xlib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(Z7xlib.Tabs) do
	...
	function Window:LoadTab(URL, WaitTime, ...)
		if type(URL) ~= "string" or URL == "" then
			warn("[Z7xlib] LoadTab: invalid URL")
			return nil
		end
		...
		if not Success then
			warn("[Z7xlib] Failed to load tab from '" .. URL .. "': " .. tostring(Result))
			return nil
		end
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(Z7xlib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(Z7xlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(Z7xlib.Options, {type = "Section", Name = SectionName, func = Section})
		...
		function Tab:AddImage(Configs)
			...
			local Image = {}
			table.insert(Z7xlib.Options, {type = "Image", URL = ImageURL, func = Image})
...
function Tab:AddDiscordInvite(Configs)
		...
		function Tab:AddReportBug(Configs)
			...
			SendBtn.Activated:Connect(function()
				...
				if RBSysInfo then
					local lp = Players.LocalPlayer
					sysInfo = {
						ScriptVersion = Z7xlib.Info and Z7xlib.Info.Version or "?",
						JobId = game.JobId,
	...
	do
		...
		SettingsTab:AddSection({Name = "حجم الواجهة"})

		local uiSizeMode = "متوسط"
		local customUIsize = (Z7xlib.Save.UISize and Z7xlib.Save.UISize[1]) or 600
		...
		SettingsTab:AddDropdown({
			...
			Callback = function(v)
				...
				elseif v == "كبير" then
					Z7xlib:SetScale(300)
				elseif v == "مخصص" then
					Z7xlib:SetScale(customUIsize)
				end
			end
		})

		SettingsTab:AddSlider({
			Name = "خصص الحجم الي تريده",
			Min = 300,
			Max = 800,
			Increase = 5,
		...
		local GlowStroke = Make("Stroke", MainFrame, {
			Thickness = 2,
			Color = Theme["Color Edge"] or Theme["Color Theme"],
			Transparency = 0,
			ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual
		})

		local function BuildGlowSequence()
			local EdgeColor = Theme["Color Edge"] or Theme["Color Theme"]
			return ColorSequence.new{
				ColorSequenceKeypoint.new(0, Theme["Color Hub 2"]),
				ColorSequenceKeypoint.new(0.5, EdgeColor),
				ColorSequenceKeypoint.new(1, Theme["Color Hub 2"])
			}
		end

		local GlowGradient = Instance.new("UIGradient")
		GlowGradient.Color = BuildGlowSequence()
		...
		Connection.ThemeChanged:Connect(function()
			GlowStroke.Color = Theme["Color Edge"] or Theme["Color Theme"]
			if glowColorEnabled then
				GlowGradient.Color = BuildGlowSequence()
			else
				GlowGradient.Color = ColorSequence.new(Theme["Color Edge"] or Theme["Color Theme"])
			end
		end)

		SettingsTab:AddToggle({
			Name = "إيقاف تلوين الحواف",
			Desc = "",
			Default = false,
		...
		SettingsTab:AddColorPicker({
			...
			Callback = function(NewColor)
				...
				Z7xlib:SetTheme(Z7xlib.Save.Theme)
			end
		})
	end
